...
export class UsuarioComponent implements OnInit, AfterViewInit {
  ...
  aplicarFiltroTabla(event: Event){
    this.filtroTabla = (event.target as HTMLInputElement).value;
    //let filterValue = (event.target as HTMLInputElement).value;// el dice const lo pondré let
    //this.dataListaUsuarios.filter = filterValue.trim().toLocaleLowerCase(); 
    this.dataListaUsuarios.filter = this.filtroTabla.trim().toLocaleLowerCase();    
  }
